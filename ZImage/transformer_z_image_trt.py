...
"""
ZImage Transformer — TensorRT-compatible version
=================================================
Changes vs original:
  [TRT-FIX-1] Real RoPE: replace complex64 ops with real cos/sin rotation
  [TRT-FIX-2] Vectorized mask: replace for-loop index assignment with arange broadcast
  [TRT-FIX-3] Remove `with torch.device("cpu"):` context manager
"""
...
class ZSingleStreamAttnProcessor:
    ...
    [TRT-FIX-1] Uses real-valued RoPE instead of complex64.
    """
    ...
    def __call__(
        ...
        # [TRT-FIX-1] Real RoPE — no complex64
        def apply_rotary_emb_real(x_in: torch.Tensor, freqs_cos: torch.Tensor, freqs_sin: torch.Tensor) -> torch.Tensor:
            """Apply rotary embedding using real-valued cos/sin rotation.
            x_in: (batch, seq, heads, head_dim)
            freqs_cos, freqs_sin: (batch_or_1, seq, 1, half_head_dim)
            """
            with torch.amp.autocast("cuda", enabled=False):
                x = x_in.float()
                # Split into pairs: (x0, x1) for rotation
        ...
        if freqs_cis is not None:
            # freqs_cis is now (batch, seq, half_head_dim) with cos and sin stacked
            # Shape: add head dim for broadcast
...
class RopeEmbedder:
    """[TRT-FIX-1] Real-valued RoPE embedder — no complex64."""
    def __init__(
        self,
        theta: float = 256.0,
    ...
    @staticmethod
    def precompute_freqs_real(dim: list[int], end: list[int], theta: float = 256.0):
        """Precompute cos/sin frequencies as real tensors instead of complex64.
        Returns list of (cos, sin) tuples, each of shape (max_len, half_dim).
        """
        # [TRT-FIX-3] Removed `with torch.device("cpu"):` — just specify device explicitly
        all_cos = []
        all_sin = []
        for i, (d, e) in enumerate(zip(dim, end)):
            ...
            # [TRT-FIX-1] Compute cos/sin in float64 (matching torch.polar precision), then truncate
    ...
    def __call__(self, ids: torch.Tensor):
        ...
        if self.freqs_cos is None:
            self.freqs_cos, self.freqs_sin = self.precompute_freqs_real(
                self.axes_dims, self.axes_lens, theta=self.theta
            )
            self.freqs_cos = [fc.to(device) for fc in self.freqs_cos]
            self.freqs_sin = [fs.to(device) for fs in self.freqs_sin]
        else:
        ...
        for i in range(len(self.axes_dims)):
            index = ids[:, i]
            result_cos.append(self.freqs_cos[i][index])
            result_sin.append(self.freqs_sin[i][index])

        # Stack cos and sin: shape (seq, 2, total_half_dim)
        # so attention processor can index [cos, sin] easily
        cos_cat = torch.cat(result_cos, dim=-1)  # (seq, total_half_dim)
        sin_cat = torch.cat(result_sin, dim=-1)
        return torch.stack([cos_cat, sin_cat], dim=-2)  # (seq, 2, total_half_dim)


# [TRT-FIX-2] Vectorized attention mask construction
def _build_attn_mask_vectorized(item_seqlens: list[int], max_seqlen: int, device: torch.device) -> torch.Tensor | None:
    """Build attention mask without for-loop index assignment (no aten.scatter).
    Returns None if all sequences have the same length.
    """
    if all(s == max_seqlen for s in item_seqlens):
        return None
    bsz = len(item_seqlens)
    seq_lens = torch.tensor(item_seqlens, dtype=torch.long, device=device)  # (bsz,)
    indices = torch.arange(max_seqlen, device=device).unsqueeze(0)  # (1, max_seqlen)
    attn_mask = indices < seq_lens.unsqueeze(1)  # (bsz, max_seqlen) bool
    return attn_mask
...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        if siglip_feat_dim is not None:
            ...
            self.siglip_refiner = nn.ModuleList(
                [
                    ZImageTransformerBlock(2000 + layer_id, dim, n_heads, n_kv_heads, norm_eps, qk_norm, modulation=False)
                    for layer_id in range(n_refiner_layers)
                ]
            )
        ...
        # [TRT-FIX-1] Use real-valued RoPE embedder
            ...
            for i in range(bsz):
                ...
                for j in range(len(size[i])):
                    if size[i][j] is None:
                        ...
                        F_dim, H, W = size[i][j]
                        ori_len = (F_dim // pF) * (H // pH) * (W // pW)
                        pad_len = (-ori_len) % SEQ_MULTI_OF
                        x_item = (
                            unified_x[cu_len : cu_len + ori_len]
                            .view(F_dim // pF, H // pH, W // pW, pF, pH, pW, self.out_channels)
                            .permute(6, 0, 3, 1, 4, 2, 5)
                            .reshape(self.out_channels, F_dim, H, W)
                        )
    ...
    def _patchify_image(self, image: torch.Tensor, patch_size: int, f_patch_size: int):
        ...
        C, F_dim, H, W = image.size()
        F_tokens, H_tokens, W_tokens = F_dim // pF, H // pH, W // pW
        image = image.view(C, F_tokens, pF, H_tokens, pH, W_tokens, pW)
        image = image.permute(1, 3, 5, 2, 4, 6, 0).reshape(F_tokens * H_tokens * W_tokens, pF * pH * pW * C)
        return image, (F_dim, H, W), (F_tokens, H_tokens, W_tokens)
    ...
    def patchify_and_embed(
        ...
        return (
            all_img_out, all_cap_out, all_img_size,
            all_img_pos_ids, all_cap_pos_ids, all_img_pad_mask, all_cap_pad_mask,
        )
    ...
    def patchify_and_embed_omni(
        ...
        return (
            all_x_out, all_cap_out, all_sig_out, all_x_size,
            all_x_pos_ids, all_cap_pos_ids, all_sig_pos_ids,
            all_x_pad_mask, all_cap_pad_mask, all_sig_pad_mask,
            all_x_pos_offsets,
    ...
    def _prepare_sequence(
        ...
        """[TRT-FIX-2] Vectorized mask construction."""
        ...
        # [TRT-FIX-2] Vectorized attention mask — no for-loop, no aten.scatter
        attn_mask = _build_attn_mask_vectorized(item_seqlens, max_seqlen, device)
    ...
    def _build_unified_sequence(
        ...
        """[TRT-FIX-2] Vectorized mask in unified sequence."""
        bsz = len(x_seqlens)
        unified = []
        unified_freqs = []
        unified_noise_mask = []
        ...
        # [TRT-FIX-2] Vectorized attention mask
        attn_mask = _build_attn_mask_vectorized(unified_seqlens, max_seqlen, device)
    ...
    def forward(
        ...
        if omni_mode:
            (
                x, cap_feats, siglip_feats, x_size,
                x_pos_ids, cap_pos_ids, siglip_pos_ids,
                x_pad_mask, cap_pad_mask, siglip_pad_mask,
                x_pos_offsets,
            ...
            (
                x, cap_feats, x_size,
                x_pos_ids, cap_pos_ids, x_pad_mask, cap_pad_mask,
