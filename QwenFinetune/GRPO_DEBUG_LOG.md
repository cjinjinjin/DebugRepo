...
- **comp1024 vs comp2048 并行对比（comp2048 steps 1-13 均值）**：

  | 指标 | comp1024 (5 steps avg) | comp2048 (13 steps avg) | 差异 |
  |------|------------------------|-------------------------|------|
  | reward | 0.444 | **0.489** | comp2048 高 10% |
  | clipped_ratio | 0.148 | **0.0035** | comp2048 近乎无截断（仅 step 11-12 偶发） |
  | frac_reward_zero_std | 0.019 | **0.0036** | comp2048 梯度更有效 |
  | KL | 0.004 | 0.005 | 相近 |
  | step_time | 10612 | 11546 | comp2048 平均慢 9%（含 step 11-12 偶发长回复） |

  **结论**：comp2048 在 reward、梯度质量上全面优于 comp1024，且速度仅慢 ~9%（偶发长回复被截断时会延长 step_time）。comp2048 是更优配置。
