...
### 10b. 方案十 comp1024 重跑结果（2026-04-05，前 4 steps）
...
- **关键发现**：
  ...
  3. **clipped_ratio 波动但未持续上升**：step 3 升到 0.22 后，step 4 回落到 0.11（低于 step 1 的 0.14），说明 step 3 是 batch 波动而非趋势
  4. **reward 出现回升迹象**：step 4 达到 0.472（本轮最高），打破了 step 1-3 的平台/下降趋势
  5. **KL 仍然极低**：step 1-2 为 0.0，step 3-4 稳定在 ~0.007，策略更新幅度很小但已开始
  6. **frac_reward_zero_std 持续下降**：0.031 → 0.016 → 0.016 → **0.0**（step 4 所有 prompt 都有有效梯度信号）
  7. **grad_norm 极其稳定**：0.057-0.087，无异常波动（对比 comp512 step 14 的 6.494 梯度爆炸）
  8. **速度**：~10600s/step（~2.9h/step），34 steps 预计 **~8 天**

- **Step 4 积极信号**：clipped_ratio 回落 + reward 新高 + frac_reward_zero_std=0，表明 comp1024 训练信号健康。之前担心的 "clipped_ratio 持续上升" 是 batch 波动，非系统性趋势。
- **状态**：comp1024 训练进行中（4/34 steps，已运行 ~24h）；comp2048 实验待启动（见 10c）
