...
### 10b. 方案十 comp1024 重跑结果（2026-04-05~08，18 steps）
...
- **训练曲线**：
  ...
  |------|--------|------|----|---------------|-------------|------------|------------|----------------------|-----------|-------------|
  | 1 | 0.4571 | -0.0011 | 0.0 | 0.1406 | 898.5 | 647.0 | 1024.0 | 0.03125 | 0.061 | 10641 |
  | 2 | 0.4575 | 0.0778 | 0.0 | 0.1641 | 904.7 | 477.5 | 1024.0 | 0.015625 | 0.087 | 10552 |
  | 3 | 0.3987 | 0.0165 | 0.0066 | 0.2188 | 915.2 | 676.0 | 1024.0 | 0.015625 | 0.057 | 10759 |
  | 4 | 0.4719 | -0.0395 | 0.0069 | 0.1094 | 907.6 | 615.0 | 1024.0 | 0.0 | 0.060 | 10597 |
  | 5 | 0.4331 | -0.1582 | 0.0068 | 0.1094 | 914.9 | 764.5 | 1024.0 | 0.015625 | 0.058 | 10570 |
  | 6 | 0.4486 | -0.0186 | 0.0069 | 0.1953 | 908.9 | 698.0 | 1024.0 | 0.0625 | 0.088 | 10439 |
  | 7 | 0.4358 | -0.0878 | 0.0067 | 0.1484 | 894.1 | 476.0 | 1024.0 | 0.09375 | 0.053 | 10612 |
  | 8 | 0.4269 | -0.0395 | 0.0074 | 0.2188 | 908.6 | 649.0 | 1024.0 | 0.03125 | 0.073 | 10765 |
  | 9 | 0.4707 | 0.0852 | 0.0075 | 0.1328 | 906.1 | 670.0 | 1024.0 | 0.015625 | 0.073 | 10511 |
  | 10 | 0.4605 | -0.0173 | 0.0069 | 0.1406 | 907.9 | 718.5 | 1024.0 | 0.0 | 0.059 | 10762 |
  | 11 | 0.4090 | 0.0154 | 0.0068 | 0.1719 | 906.4 | 608.0 | 1024.0 | 0.046875 | 0.071 | 11257 |
  | 12 | 0.3717 | -0.0182 | 0.0082 | 0.2813 | 913.1 | 564.5 | 1024.0 | 0.0625 | 0.062 | 10960 |
  | 13 | 0.4563 | 0.0371 | 0.0062 | 0.0859 | 895.8 | 749.0 | 1024.0 | 0.015625 | 0.057 | 10317 |
  | 14 | 0.4022 | 0.0153 | 0.0066 | 0.1797 | 919.3 | 751.5 | 1024.0 | 0.078125 | 0.062 | 10944 |
  | 15 | 0.4437 | 0.0701 | 0.0069 | 0.1406 | 905.1 | 705.0 | 1024.0 | 0.0625 | 0.065 | 10656 |
  | 16 | 0.4068 | 0.0361 | 0.0067 | 0.2188 | 919.5 | 739.0 | 1024.0 | 0.078125 | 0.061 | 10655 |
  | 17 | 0.4518 | 0.0634 | 0.0077 | 0.1563 | 912.4 | 732.0 | 1024.0 | 0.015625 | 0.062 | 10648 |
  | 18 | 0.4687 | 0.0053 | 0.0067 | 0.1094 | 906.6 | 768.5 | 1024.0 | 0.0 | 0.061 | 10563 |
...
- **关键发现**：
  ...
  2. **模型实际需要 ~900 tokens**：mean_length 稳定在 894-919，min_length 476-769，说明 512 根本不够
  3. **clipped_ratio 波动在 0.09-0.28 之间**：18 步均值 ~0.160，step 12 最高 0.28
  4. **reward 在 0.37-0.47 之间平台波动**：18 步均值 ~0.435，远超 SFT baseline（0.211），但无明确上升趋势
  5. **KL 稳定**：step 1-2 为 0.0，step 3+ 稳定在 ~0.007，策略更新幅度极小
  6. **frac_reward_zero_std**：step 14/16 升到 0.078，整体无恶化趋势
  7. **grad_norm 极其稳定**：0.053-0.088，无异常波动
  8. **速度**：~10650s/step（~2.96h/step），已运行 107h（4.5天），剩余 16 steps ~47h

- **18 步总结**：reward 稳定在 ~0.43 平台，无上升趋势。后半程（steps 11-18 均值 0.426）低于前半程（steps 1-10 均值 0.447），略有下降但在波动范围内。step 12 出现最低 reward 0.372，同时 clipped_ratio 最高 0.281。训练状态健康但 v1 reward 格式分占比过高导致模型无有效学习信号。
- **已保存 checkpoint**：`checkpoint-10`（`.../grpo_zero2_qlora_novllm_canary_len2048_comp1024_gen2/v0-20260404-151523/checkpoint-10`）
- **状态**：comp1024 训练进行中（18/34 steps，已运行 ~107h）；将在此机器上停止 comp1024 后启动 reward v2 训练
- **决定**：comp1024 使用 v1 reward 已确认 reward hacking 问题，继续训练无意义。停止后在同一机器启动 `run_grpo_reward_v2.sh`
...
- **训练曲线**：
  ...
  | 7 | 0.4882 | -0.0843 | 0.0063 | 0.0 | 909.5 | 738.0 | 1144.0 | 0.0 | 0.057 | 11026 |
  | 8 | 0.4924 | 0.0037 | 0.0072 | 0.0 | 905.4 | 712.0 | 1133.5 | 0.0 | 0.066 | 11132 |
  | 9 | 0.4955 | -0.0182 | 0.0066 | 0.0 | 902.0 | 685.0 | 1105.0 | 0.0 | 0.058 | 10909 |
  | 10 | 0.5024 | 0.0046 | 0.0062 | 0.0 | 912.1 | 686.0 | 1146.5 | 0.0 | 0.058 | 11246 |
  | 11 | 0.4785 | 0.0925 | 0.0064 | 0.0078 | 916.2 | 741.0 | 1678.0 | 0.031 | 0.062 | 13983 |
  | 12 | 0.4825 | -0.1390 | 0.0067 | 0.039 | 945.3 | 705.0 | 2048.0 | 0.016 | 0.055 | 15567 |
  | 13 | 0.5041 | 0.1141 | 0.0062 | 0.0 | 903.8 | 689.0 | 1108.5 | 0.0 | 0.061 | 10833 |
...
- **关键发现**：
  ...
  6. **frac_reward_zero_std 几乎始终为 0.0**：仅 step 11（0.031）和 step 12（0.016）出现极少数无方差 prompt，但大部分 steps 梯度 100% 有效
  7. **"Could not estimate tokens" 警告**：无害，transformers 对该模型架构缺少 FLOPs 估算，不影响训练
  8. **Step 3 grad_norm 小幅上升到 0.147**：后续 steps 回落并稳定在 0.055-0.066 范围，属正常波动
  9. **显存 step 3+ 稳定在 73.15 GiB**，仍有 ~7 GiB 余量
  10. **Step 11-12 首次出现截断（clipped_ratio > 0）**：step 11 max_length=1678, clipped_ratio=0.78%；step 12 max_length=2048（触及上限！），clipped_ratio=3.9%，step_time 相应增加到 14k-15.5k 秒。但 step 13 完全恢复（clipped_ratio=0, step_time=10833s），说明是偶发的长回复探索，非系统性退化
  11. **Reward 缓慢上升趋势初现**：steps 1-6 均值 ~0.484，steps 7-13 均值 ~0.492（+1.6%），step 10 和 step 13 都突破 0.50。虽上升幅度小，但方向一致
- **预期修正**：
  - ~~step_time 15000-20000s~~ → 实际 ~11000s（与 comp1024 几乎持平）
  - ~~OOM 风险~~ → 不存在（70.3 GiB，余量 ~10 GiB）
  - ~~34 steps 5-7 天~~ → 预计 **~8.6 天**（与 comp1024 相同）
- **状态**：训练进行中（13/34 steps，已运行 ~88h），checkpoint-1 至 checkpoint-13 已保存

- **comp1024 vs comp2048 并行对比（comp1024 18 steps, comp2048 13 steps）**：

  | 指标 | comp1024 (18 steps avg) | comp2048 (13 steps avg) | 差异 |
  |------|-------------------------|-------------------------|------|
  | reward | 0.435 | **0.489** | comp2048 高 12% |
  | clipped_ratio | 0.160 | **0.0035** | comp2048 近乎无截断（仅 step 11-12 偶发） |
  | frac_reward_zero_std | 0.035 | **0.0036** | comp2048 梯度更有效 |
  | KL | 0.005 | 0.005 | 相近 |
  | step_time | 10648 | 11546 | comp2048 平均慢 8% |

  **结论**：comp2048 在 reward、梯度质量上全面优于 comp1024，且速度仅慢 ~8%。comp1024 后期（steps 11-18）reward 均值 0.426 低于前期（steps 1-10）0.447，出现轻微退化；而 comp2048 保持稳定。comp2048 是更优配置。
  
  **注**：两个实验均使用 v1 reward（格式分占比 69%），已确认存在 reward hacking 问题。v1 reward 下继续训练无意义，需切换到 reward v2。comp1024 将被停止，在同一机器上启动 reward v2 训练。
