...
### 10b. 方案十 comp1024 重跑结果（2026-04-05~06，前 7 steps）
...
- **关键发现**：
  ...
  3. **clipped_ratio 波动在 0.11-0.22 之间**：均值 ~0.15，无上升趋势
  4. **reward 在 0.40-0.47 之间平台波动**：7 步均值 ~0.446，远超 SFT baseline（0.211），但无明确上升趋势
  5. **KL 仍然极低**：step 1-2 为 0.0，step 3-4 稳定在 ~0.007，策略更新幅度很小但已开始
  6. **frac_reward_zero_std 有上升趋势**：step 6 为 0.0625，step 7 为 0.09375（部分 prompt 所有 generation 获得相同 reward，梯度无效）
...
- **状态**：训练进行中（3/34 steps），checkpoint-1/2/3 已保存

- **comp1024 vs comp2048 并行对比（同 step 数）**：

  | 指标 | comp1024 (step 3) | comp2048 (step 3) | 差异 |
  |------|-------------------|-------------------|------|
  | reward | 0.399 | **0.454** | comp2048 高 14% |
  | clipped_ratio | 0.219 | **0.0** | comp2048 无截断 |
  | frac_reward_zero_std | 0.016 | **0.0** | comp2048 梯度更有效 |
  | KL | 0.0066 | 0.0061 | 相近 |
  | step_time | 10759 | 11212 | comp2048 仅慢 4% |
