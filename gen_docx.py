...
add_para('在 OaaS_LLMTemplate 仓库创建个人分支（如 jinjinchen/ZImage-v1），完成以下工作后再推送代码触发 CI 构建：')
...
add_para('在 OaaS_LLMTemplate 仓库创建个人分支（如 jinjinchen/ZImage-v1），修改以下文件：')
add_table(
    ['文件', '说明'],
    [
...
add_note('模板定制是可选步骤。先检查 OaaS LLM Template 仓库原始模板是否已满足需求。对于纯文本 LLM 推理任务，通常无需定制。')
add_table(
    ['定制需求', '说明', '修改文件'],
    [
...
add_note('注意：Step 2+3（上传 ckpt + 数据迁移）和 Step 4（镜像构建）可以同步进行，互不依赖。')
add_para('本地开发和测试完成后，将代码推送到 OaaS_LLMTemplate 仓库的个人分支，CI pipeline 会自动构建 Docker 镜像。')
...
# Save
output_path = r"C:\Users\jinjinchen\OneDrive - Microsoft\DLIS_Model_Deployment_Guide_v5_2.docx"
