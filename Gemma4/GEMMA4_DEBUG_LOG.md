...
**已确认**：从 deployment #7 容器日志（DLIS portal）找到根因，见下方。

---

## Deployment #7 — CUDA_VISIBLE_DEVICES UUID 问题（2026-04-20）

### 现象

- 容器不再 crash loop（#6 修复生效，runner=None 时不抛异常）
- 但模型仍未加载：`FATAL: OaasWrapper runner is None`
- `/tmp` writable mirror 创建成功，`_opt` 目录被检测到
- vLLM 引擎初始化失败

### 根因

DLIS 设置 `CUDA_VISIBLE_DEVICES` 为 GPU UUID 格式：
```
CUDA_VISIBLE_DEVICES=GPU-405664ab-9200-2adf-a1f1-f451359bc564
```

vLLM 在 `platforms/interface.py:219` 的 `device_id_to_physical_device_id()` 中执行：
```python
return int(physical_device_id)  # → ValueError: invalid literal for int()
```

**完整错误链**：
1. `fused_moe` import → `w8a8_utils` → `cutlass_fp8_supported()`
2. → `get_device_capability()` → `device_id_to_physical_device_id()`
3. → `int("GPU-405664ab-...")` → **ValueError**
4. → `Gemma4ForConditionalGeneration` inspection 失败
5. → `ModelConfig` validation error: `Model architectures ['Gemma4ForConditionalGeneration'] failed to be inspected`
6. → `Failed to load sync VLLM engine`

### 修复（deployment #8 准备）

`model.py` 中，OaasWrapper 初始化之前，检测并转换 UUID 格式的 `CUDA_VISIBLE_DEVICES`：

```python
cuda_env = os.environ.get("CUDA_VISIBLE_DEVICES", "")
if cuda_env and not cuda_env.replace(",", "").isdigit():
    gpu_count = len(cuda_env.split(","))
    new_val = ",".join(str(i) for i in range(gpu_count))
...
#### 尝试 2 验证结果（2026-04-30）

新镜像 `20260429-0930-merge` 构建日志确认：
- `cuda-compat-12-9 is already the newest version (575.57.08-0ubuntu1)` — 基础镜像 `vllm/vllm-openai` 已自带
- `LD_LIBRARY_PATH=/usr/local/cuda-12.9/compat` 已设置
- 但容器仍然 crash loop：`[Init] vLLM engine failed after 10.8s: Engine core initialization failed`

**cuda-compat 前向兼容无法桥接 driver 535 → CUDA 12.9 的差距。** NVIDIA Forward Compatibility 设计上只支持小版本跨越（如 12.4→12.6），不支持从 535 驱动（CUDA 12.2）跳到 CUDA 12.9。

#### 附加发现：protobuf 版本冲突

构建日志显示 `requirements-common.txt` 强制 `protobuf<5.0.0`（安装了 4.25.9），但 vLLM 0.19.1 运行时需要 `protobuf>=5.29.6`。这是潜在的二级问题，但不是当前 crash 的根因。

### ~~最终结论（2026-04-30 早）~~ — 已修正，见下方

~~**驱动 535.129.03 是死结，所有镜像级修复方案均不可行**~~ — 此结论部分有误。

| 方案 | 可行性 | 原因 |
|------|--------|------|
| cuda-compat 前向兼容 | ⚠️ 重新评估中 | DLIS 团队（Gavroche Yao）称应支持 535+，NVIDIA 文档也确认 |
| 降级到 CUDA 12.2 | ❌ | Gemma4 需要 transformers≥5.5 → torch≥2.6 → 最低 CUDA 12.4 |
| 降级到 CUDA 12.4 | ❌ | CUDA 12.4 需要驱动 ≥550.54，535 仍然不够 |
| 从 devel 镜像源码构建 | ❌ | 编译环境的 CUDA 版本不影响运行时对驱动的要求，torch cu124 仍需驱动 ≥550 |

---

### 尝试 3：Mariner A100 (driver 550) + 调试（2026-04-30）

**环境：** `DLIS-Linux-Pair,DLIS-Mariner-GPU-A100_23Cores`（Polaris test）

**改动：**
- `run.sh`: 注释 `unset CUDA_VISIBLE_DEVICES`，添加 CUDA compat 调试打印（commit `062aece`）
- 切换到 Mariner A100 机型（driver 550.54.15）

**结果：部分进展，仍失败**

✅ 驱动已升级 — GPU 识别正常：
```
[Init] GPU: 0, NVIDIA A100 80GB PCIe, 81920 MiB, 81037 MiB, 0 MiB, 31, 550.54.15, 8.0
```

✅ cuda-compat 库正确加载：
```
libnvidia-gpucomp.so.575.57.08 => /usr/local/cuda-12.9/compat/
libnvidia-ptxjitcompiler.so.1 => /usr/local/cuda-12.9/compat/
```

❌ vLLM 引擎初始化仍失败：
```
[Init] vLLM engine failed after 10.5s: Engine core initialization failed. See root cause above. Failed core proc(s): {}
```

**分析 DLIS host 日志（DLMSLog_Common.log）发现两个问题：**

1. **`ShmSize: 1073741824`（1GB）** — DLIS 默认给容器 1GB shared memory。vLLM V1 使用多进程 + 共享内存 IPC，1GB 可能不够导致子进程被 kill，`Failed core proc(s): {}` 无错误信息就是因为子进程 silent crash。Polaris 配置中无 ShmSize 参数，需联系 DLIS 团队修改。

2. **`VLLM_USE_V1=0`** — 我们设了禁用 V1 引擎，但 vLLM 0.19.1 已废弃 V0，traceback 仍走 `vllm/v1/engine/` 路径，说明此变量被忽略。应删除此配置。

3. **`DisableModelLogging=true`** — 容器 stdout/stderr 被禁止输出，导致看不到 vLLM 的真正报错。需改为 `false`。

**容器创建参数摘要（来自 DLMSLog_Common.log）：**
```
Image: 429234011_gemma4-awq-v1...base:1
Cmd: ./dlis_model/run.sh http
GPU: GPU-c4235462-cd61-f6d8-4b54-0947a56a0532
ShmSize: 1073741824 (1GB ← 太小)
Memory: 83886080000 (80GB)
CpusetCpus: 16 cores
RestartPolicy: always
```

### 对比分析：旧运行 (4/20) vs 新运行 (4/29)

分析 `Q:\data\Base\`（4/20 运行）和 `Q:\data\Base_Mariner\`（4/29 运行）的日志：

| 对比项 | 旧 (4/20, `Base`) | 新 (4/29, `Base_Mariner`) |
|--------|---------------------|---------------------------|
| 机型 | `DLIS-Linux-A100_23Cores` | `DLIS-Mariner-GPU-A100_23Cores` |
| 环境变量 | `GPU_MEMORY_UTILIZATION=0.7` | `VLLM_USE_V1=0;GPU_MEMORY_UTILIZATION=0.9;ENFORCE_EAGER=true` |
| ModelDataPath | `gemma4-vllm` | `gemma4-vllm-v2` |
| Docker 镜像 | `20260421-0530-merge` | `20260429-0930-merge` |
| MaxMemoryUsageInMB | 31000 | 80000 |
| 模型加载 | ✅ 成功（~3min 后） | ❌ 失败（engine init crash） |
| 推理结果 | ❌ 全 Failed（空输出） | N/A |

**关键发现：**

1. **`VLLM_USE_V1=0` 是致命问题** — 旧运行**没有这个变量**，vLLM 成功加载。新运行加了此变量后 engine 崩溃。vLLM 0.19.1 已完全移除 V0，设置此变量可能导致内部状态矛盾。

2. **旧运行 ModelDataPath 一开始 404** — ADLS 路径 `gemma4-vllm` 最初不存在，重试了 ~3 分钟后才找到（可能是 upload 延迟或路径传播延迟），最终成功下载 4 个 safetensors 模型文件。

3. **旧运行推理全部 Failed** — 62 个 warmup query + 63 个 test query 全部返回 `"Status": "Failed"`，`raw_output` 为空，`step1_raw` = `"</Scene5>"`。延迟稳定在 ~1914ms，说明 vLLM 确实在做推理但 **prompt 格式或 dlis_inter.py 请求处理有问题**。Polaris 结果显示 `SignoffResult: Pass`（100% 成功率），因为 DLIS 层面 HTTP 响应码为 0（成功），它不检查应用层 JSON 的 Status 字段。

4. **推理结果问题可能是 dlis_inter.py 的 prompt 构造** — 响应中的 `step1_raw: "</Scene5>"` 暗示模型输出了一个截断或不完整的 XML/场景标签，可能是 prompt template 不匹配 Gemma4 的格式要求。

### Kusto 日志分析（2026-04-30）

从 Kusto 查询到的容器初始化日志：
```
[Init] Loading vLLM engine...
| VLLM_USE_V1=0
| GPU_MEMORY_UTILIZATION=0.9
| MAX_MODEL_LEN=2048
| ENFORCE_EAGER=true
| CUDA_VISIBLE_DEVICES=not set
| /dev/shm=total=1024MB free=1024MB
```

**4 个关键问题：**

#### 1. `CUDA_VISIBLE_DEVICES=not set` ⚠️ 新发现 P0

GPU 对容器不可见。DLIS host 传递了 `GPU-c4235462-...` UUID 给 Docker，但容器内 `CUDA_VISIBLE_DEVICES` 为空。

**排查 run.sh 历史：**
- commit `c146cf2`：曾**添加** `unset CUDA_VISIBLE_DEVICES`（错误操作）
- commit `062aece`（我们的修复）：注释掉了 `unset CUDA_VISIBLE_DEVICES`，加了 CUDA compat 调试输出

**可能原因：**
1. Docker 镜像 `20260429-0930-merge` 可能**没有包含我们的 run.sh 修复**（构建时间早于 push？）
2. `source ~/.bashrc` 可能清除了 `CUDA_VISIBLE_DEVICES`
3. DLIS host 传入的 GPU UUID 格式 vs Docker `--gpus` 参数不匹配

**验证方法：** 在 run.sh 顶部（`source ~/.bashrc` 之前）打印 `CUDA_VISIBLE_DEVICES`，与 `source ~/.bashrc` 之后的值对比，确认是否被 bashrc 覆盖。

#### 2. `VLLM_USE_V1=0` — 已知 P0

vLLM 0.19.1 已完全移除 V0 引擎，此变量导致引擎初始化矛盾。必须从 Polaris EnvironmentVariables 中删除。

#### 3. `/dev/shm=total=1024MB` — P1→P2

vLLM V1 使用多进程 + 共享内存 IPC。1GB shm 可能导致子进程被 OOM kill，表现为 `Failed core proc(s): {}`（无错误信息的 silent crash）。DLIS 硬编码 `ShmSize: 1073741824`，用户不可配置。

#### 4. `MAX_MODEL_LEN=2048` — 信息

当前设置 2048 token，对于调试阶段足够。上线后需根据实际 prompt 长度调整。
