#!/bin/bash
# Gemma 4 AWQ 4-bit zero-shot evaluation on DPO 196 samples
# Compares AWQ quantized model against BF16 results (95.9% compliant, avg 89.9 words)
#
# Usage:
#   bash Gemma4/eval_gemma4_awq_zeroshot.sh
#   bash Gemma4/eval_gemma4_awq_zeroshot.sh [model_path]
...
if [ ! -d "${MODEL_ID}" ]; then
    MODEL_ID="./gemma-4-26B-A4B-it-AWQ-4bit"
fi
...
OUTPUT_FILE="${VC_RESULTS}/gemma4_awq_zeroshot_eval.jsonl"
REPORT_FILE="${VC_RESULTS}/gemma4_awq_zeroshot_report.json"
...
if [ ! -d "${MODEL_ID}" ]; then
    ...
    echo "  Download first: HF_TOKEN=hf_xxx bash Gemma4/download_model_awq.sh"
    exit 1
fi
