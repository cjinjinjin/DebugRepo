#!/bin/bash
# Gemma 4 Random200 inference: Full-Prompt + vLLM (AWQ 4-bit)
# TSV → JSONL → Full-Prompt vLLM inference → evaluate → T2I extraction
#
# Usage:
#   bash Gemma4/eval_gemma4_random200_full_prompt_vllm.sh
#   bash Gemma4/eval_gemma4_random200_full_prompt_vllm.sh [model_path] [tensor_parallel_size]
...
OUTPUT_FILE="${VC_RESULTS}/gemma4_random200_full_prompt_vllm_eval.jsonl"
REPORT_FILE="${VC_RESULTS}/gemma4_random200_full_prompt_vllm_report.json"

mkdir -p "${VC_RESULTS}"
mkdir -p "$(dirname "${INFER_INPUT}")"

echo "============================================"
echo "Gemma 4 Random200 — Full-Prompt + vLLM (AWQ 4-bit)"
echo "============================================"
echo "Model        : ${MODEL_ID}"
...
# ── Step 2: Full-Prompt vLLM inference ────────────────────────────────────
echo ""
echo "[INFO] Starting Full-Prompt vLLM inference (TP=${TP_SIZE}) ..."
python "${SCRIPT_DIR}/inference_gemma4_full_prompt_vllm.py" \
    --model_id "${MODEL_ID}" \
...
T2I_FILE="${VC_RESULTS}/gemma4_random200_full_prompt_vllm_t2i.txt"
